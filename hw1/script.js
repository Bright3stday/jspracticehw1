...
function addTransactionDOM(transaction) {
  ...
  custBalance = Math.abs(transaction.deposit) - Math.abs(transaction.loan); //calculates the customer balance 

  (custBalance>0)? balance_item.classList.add('plus'):balance_item.classList.add('minus'); //ternary operator to check if cust has negative or positve balance and adds the relevant class to it
  
  //below uses ternary operator to output the correct message to the user depending on if they have positive or negative balance
  (custBalance>0)? balance_item.innerHTML = `
...
function filterTransaction(e) {
  ...
  list.innerHTML = '';
  reco.innerHTML = '';
  document.getElementById('yourBalance').innerHTML= "Your Balance"; //adds relevant text to html tag so that user can see relevant information after logging in
  document.getElementById('depHeader').innerHTML= "Deposit";
  document.getElementById('loanHeader').innerHTML= "Loan";
  document.getElementById('details').innerHTML= "Balance Details";
  document.getElementById('details').style.borderBottom= "1px solid #bbb";
  document.getElementById('logIn').innerHTML = `Welcome ${custname.value.toUpperCase()}!`;
  document.getElementById('addShadow').style.boxShadow= "var(--box-shadow)";
